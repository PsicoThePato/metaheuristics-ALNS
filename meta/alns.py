...
class ALNSIter:
    ...
    def do_alns_iteraction(self, destruction_parameter: float, state: definitions.ProblemDefinition.state) -> definitions.ProblemDefinition.state:
        ...
        """
        incomplete_state, dh_idx = self._destroy(destruction_parameter, state) # retorna estadoincompleto/grafo incompleto e indice da heuristtica
        new_state, rh_idx = self._repair(incomplete_state) # novo estado completo/grafo completo e o indice da heuristica utilizada
        new_state_score = self.scoring_function(new_state) # irá retornar o quão bom é o novo  grafo segundo algum critério
        current_state_score = self.scoring_function(self.current_state)
		isaccepted = self.acceptance_function(StateTransition(self.current_state, new_state)) # irá  retornar um booleando dizendo se a nova solução foi aceita
        success_increment = 0
